...
export function mapProductToApi(localProduct) {
  ...
  return {
    ...
    originalPrice: localProduct.originalPrice || null,
...
export async function checkWishlist() {
  return request('/api/wishlist/check');
}

// ── Search ──────────────────────────────────────────────────────────────────

export async function saveSearch(query, sessionId) {
...
export async function fetchRecentSearches(sessionId) {
  const params = sessionId ? `?sessionId=${sessionId}` : '';
  return request(`/api/search/recent${params}`);
}

// ── Image Search ────────────────────────────────────────────────────────────

export async function searchByImage(imageData, category) {
  const data = await request('/api/products/search/image', {
    method: 'POST',
    body: JSON.stringify({ imageData, category }),
  });
  return (data.results || []).map(mapProductFromApi);
}
